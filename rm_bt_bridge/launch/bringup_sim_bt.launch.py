...
def generate_launch_description():
    ...
    spin_bypass = LaunchConfiguration('spin_bypass')
    ...
    declare_spin_bypass_cmd = DeclareLaunchArgument(
        'spin_bypass',
        default_value='False',
        description='Bypass fake_vel_transform spin, use raw cmd_vel for manual control')
    ...
    referee_params_path = os.path.join(
        get_package_share_directory('rm_referee_simulator'), 'config', 'referee_params.yaml')

    referee_simulator_node = Node(
        package='rm_referee_simulator',
        executable='referee_simulator_node',
        output='screen',
        parameters=[referee_params_path]
    )

    #################################### behavior-tree decision start ####################################
    cod_behavior_pose_yaml = os.path.join(
        get_package_share_directory('rm_bt_bridge'), 'config', 'cod_behavior_pose.yaml')
    cod_bt_path = os.path.join(
        get_package_share_directory('cod_behavior'), 'cod_bt', 'singlenav_basic.xml')

    # 仿真数据源桥接：referee RobotStatus -> /SerialReceiveData
    referee_to_serial_bridge = Node(
        package='rm_bt_bridge',
        executable='referee_to_serial_bridge',
        output='screen',
        parameters=[{
            'zone_status_default': False,
            'is_defence_default': False,
            'is_attack_default': False,
            'is_recover_default': False,
            'self_status_default': False,
        }]
    )

    # 导航接口桥接：/goal_pose -> Nav2 navigate_to_pose action
    ...
    # COD 行为树决策节点（替代 decision_node + referee_executor）
    cod_behavior_node = Node(
        package='cod_behavior',
        executable='tree_1',
        name='cod_behavior',
        output='screen',
        parameters=[cod_behavior_pose_yaml, {'cod_bt_path': cod_bt_path}]
    )
    ##################################### behavior-tree decision end #####################################
    ...
    ld.add_action(declare_spin_bypass_cmd)
    ...
    ld.add_action(referee_simulator_node)
    ld.add_action(referee_to_serial_bridge)
    ld.add_action(goal_pose_bridge)
    ld.add_action(cod_behavior_node)
